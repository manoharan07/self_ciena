...
public class EventReceiverThread implements Runnable {
	...
	private Integer retryAfterMs = 5000; // default 5 seconds

	public EventReceiverThread(String deviceIP, String userName, String password, Integer port,Integer retryAfterMs) {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				HttpConnection conn = new HttpConnection(this.deviceIP, this.port, this.userName, this.password);
				...
				if (urlConn != null) {
					...
					logger.info(
							"device IP : " + this.deviceIP + " received token : " + token + " : cookie : " + cookie);

					String tokenKey = "-xsrf-";
					String uri = String.format("wss://%s:%s/status?-xsrf-=%s", this.deviceIP, this.port, token);
					logger.info("uri : " + uri);
					...
					try {
						CienaEventClient c = tryConnect(uri, cookie, draft);
					...
					//logger.info("Will try connecting again with " + this.deviceIP + " after 10 minutes");
					//Thread.sleep(600000); // wait for 10 minutes and try again.
				Thread.sleep(retryAfterMs); // wait 5 seconds and try again.
	...
	private CienaEventClient tryConnect(String uri, String cookie, Draft_6455 draft) throws Exception {
		Map<String, String> httpHeaders = new HashMap<String, String>();
		httpHeaders.put("Cookie", cookie);
		CienaEventClient c = new CienaEventClient(new URI(uri), draft, httpHeaders,
		...
		//System.out.println("################### connectionLostTimeout : "+c.getConnectionLostTimeout());
		//c.setConnectionLostTimeout(arg0);
		c.connectBlocking();
