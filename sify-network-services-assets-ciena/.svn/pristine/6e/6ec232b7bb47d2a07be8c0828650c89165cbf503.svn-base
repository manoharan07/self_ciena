...
public class CienaReportServiceAmazonHealth extends AssetsAbstractService {
	...
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger("amazonhealthreportlog");

//	private DbConnection dbObj = null;
//	private Connection connection = null;
	
	String ipList6500[] = {
			"100.70.123.158","100.70.123.150","100.70.123.159","100.70.123.151",
			"100.70.123.133","100.70.123.77","100.70.123.134","100.70.123.78"
			};
	
	String ipListWS[] = {
			"100.70.123.144","100.70.123.145","100.70.123.146","100.70.123.147","100.70.123.148",
			"100.70.123.152","100.70.123.153","100.70.123.154","100.70.123.155","100.70.123.156",
			"100.70.123.72","100.70.123.73","100.70.123.74","100.70.123.75","100.70.123.76",
			"100.70.123.128","100.70.123.129","100.70.123.130","100.70.123.131","100.70.123.132"
			};
	...
	public CienaReportServiceAmazonHealth() {
		super();
...
*/		
	...
	public void init() {
		...
		CienaReportServiceAmazonHealth service = new CienaReportServiceAmazonHealth();
		service.setServiceName("CienaReportServiceAmazonHealth");
		...
		// inventory
		 ...
		 service.setLivePollThreadCound(cienaProperties.get("ciena.amazonhealthreport..thread.count",10));
		 
		
		 
		logger.info("CienaReportServiceAmazonHealth  details added");
		...
		service.setInventoryCron(CienaProperties.get("ciena.amazonhealthreport.cron"));
	...
	public static void main(String[] args) {
		PropertyConfigurator.configure("config" + File.separator + "log4j_amazonhealth.properties");
		CienaReportServiceAmazonHealth service = new CienaReportServiceAmazonHealth();
		service.init();
...
*/		
		...
		try {
				...
				for(int i=0;i<jsonArray.length();i++) {
					...
					InventoryDeviceDetails perfData=new InventoryDeviceDetails(ip) {
						String sql="INSERT INTO optical_reports.amazon_6500_perf_current(node_name, node_ip, port_id, parameter_name, parameter_value, collection_date, updated_time) "
				...
				logger.info("@@@@@@@@@@2 working status : "+jsonObjectOptMon);
				...
				//	String remoteStandard=null;
					...
					InventoryDeviceDetails perfData=new InventoryDeviceDetails(ip) {
						String sql="INSERT INTO optical_reports.amazon_6500_wps_status	(node_name, node_ip, facility_id, scheme, memeber_type, remote_standard, "
	...
	public ArrayList<InventoryDeviceDetails> retrieve6500CurrentAlarms(String ip, String nodeName, Map<String,String> sessionMap) {
		...
		for(int i=0;i<jsonArrayAlarms.length();i++) {
			...
			logger.info("6500 Alarm ########## "+jsonObj.get("aid")+", condtype : "+jsonObj.get("condtype"));
			...
			InventoryDeviceDetails perfData=new InventoryDeviceDetails(ip) {
				String sql="INSERT INTO optical_reports.amazon_6500_alarms_current(node_name, node_ip, instance_aid, aid_type, severity, condition_type, service_affecting, "
	...
	public ArrayList<InventoryDeviceDetails> retrieve6500pm(String ip, String nodeName, Map<String,String> sessionMap) {
		...
		for (int i = 0; i < jsonArrayCounts.length(); i++) {
			JSONObject jsonObj = jsonArrayCounts.getJSONObject(i);
				...
				logger.info("@@@@@@@@@@@@ "+portId+","+parameter+","+value);
				
				String location = jsonObj.getString("locn");
				String direction = jsonObj.getString("dirn");

				InventoryDeviceDetails perfData = new InventoryDeviceDetails(ip) {
					String sql = "INSERT INTO optical_reports.amazon_6500_pm(node_name, node_ip, port_id, parameter_name, parameter_value, location, direction, collection_date, updated_time) "
				...
				//	String rxPort="";
					...
					*/	
					...
					InventoryDeviceDetails perfDataTx=new InventoryDeviceDetails(ip) {
						String sql="INSERT INTO optical_reports.amazon_ws_perf_current(node_name, node_ip, port_id, parameter_name, parameter_value, collection_date, updated_time) "
	...
	public ArrayList<InventoryDeviceDetails> retrievePerformanceHistoryOptical(String ip) {
		...
		try {
				...
				for(int i=0;i<allBins.length();i++) {
					...
					InventoryDeviceDetails perfData=new InventoryDeviceDetails(ip) {
						String sql="INSERT INTO optical_reports.amazon_ws_perf_history (node_name, node_ip, configured_mode, interface_name, interface_type, profile_type, "
								+ "collection_start_time, collection_end_time, configured_bin_count, admin_state, oper_state, "
								+ "bin_collection_start_time, bin_collection_end_time, "
	...
	public ArrayList<InventoryDeviceDetails> retrieveActiveAlarmsWS(String ip) {
		...
		try {
				...
				for(int i=0;i<alarmsArray.length();i++) {
					...
					InventoryDeviceDetails perfData=new InventoryDeviceDetails(ip) {
						String sql="INSERT INTO optical_reports.amazon_ws_alarms_current(node_name, node_ip, alarm_instance_id, acknowledged, intermittent, "
...
/*		DateFormat f = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		logger.info("Polled date being used : "+dateStr);
*/		
		//String dateStr = "2020-07-12";
		//String dateStr = "2020-07-12 08:00:00"; //with time
		...
		dateStrFileName = dateStr.replaceAll(" ", "_").replaceAll(":", "_");
		
		dateStrFileName = dateStrFileName.substring(0,dateStrFileName.length()-3);
		
		String excelPath = filePath + File.separator +  "amazon_health_report_"+dateStrFileName+".xlsx";
		
		logger.info("File name used : "+excelPath);
		logger.info("Amazon Health Report generation started for "+dateStr);
		FileOutputStream fileOut = new FileOutputStream(excelPath);
	    XSSFWorkbook wworkbook=createWorkBook();
	    
		create6500PerformanceCurrent(wworkbook,dateStr);
		
		create6500PerformanceSpanLoss(wworkbook,dateStr);
		
		
		
		createWsPerformanceCurrent(wworkbook,dateStr,"Wave Server BOM53-BOM59-PP4","'100.70.123.144','100.70.123.145','100.70.123.146','100.70.123.147','100.70.123.148'",0);
		
		createWsPerformanceCurrent(wworkbook,dateStr,"Wave Server BOM53-BOM59-PP4","'100.70.123.152','100.70.123.153','100.70.123.154','100.70.123.155','100.70.123.156'",7);
		
		
		
		createWsPerformanceCurrent(wworkbook,dateStr,"Wave Server BOM53-BOM59-PP5","'100.70.123.72','100.70.123.73','100.70.123.74','100.70.123.75','100.70.123.76'",0);
			
		createWsPerformanceCurrent(wworkbook,dateStr,"Wave Server BOM53-BOM59-PP5","'100.70.123.128','100.70.123.129','100.70.123.130','100.70.123.131','100.70.123.132'",7);
		
		
		
		for(int i=0;i<10;i++) {
			createWsPerformanceHistory(wworkbook, dateStr, "BOM53-BOM59-PP4-WS-15m&24 Hr PM", ipListWS[i],0+i*4);
			
			
		}
		
		XSSFSheet sheet = wworkbook.getSheet("BOM53-BOM59-PP4-WS-15m&24 Hr PM");
		...
		for(int i=10;i<20;i++) {
			createWsPerformanceHistory(wworkbook, dateStr, "BOM53-BOM59-PP5-WS-15m&24 Hr PM", ipListWS[i],0+(i-10)*4);
			
			
		}
		
		sheet = wworkbook.getSheet("BOM53-BOM59-PP5-WS-15m&24 Hr PM");
		
        for (int i = 0; i < 50; i++) {
            sheet.autoSizeColumn(i);
        }
		
		create6500pm(wworkbook,dateStr);
		
		createWsCurrentAlarms(wworkbook,dateStr);
		
		create6500CurrentAlarms(wworkbook,dateStr);
		
		create6500HistoryAlarms(wworkbook,dateStr);
		
		createOPSStatus(wworkbook,dateStr);
		
		wworkbook.write(fileOut);
		fileOut.flush();
		fileOut.close();
		logger.info("Amazon Health Report generation completed for "+dateStr);
		} catch(Exception e) {
			e.printStackTrace(System.out);
		}
	...
	public void create6500PerformanceCurrent(XSSFWorkbook wworkbook,String dateStr) {
		create6500PerformanceCurrent(wworkbook,dateStr,"BOM53",0);
		create6500PerformanceCurrent(wworkbook,dateStr,"BOM59",9);
	}
	
	public void create6500PerformanceCurrent(XSSFWorkbook wworkbook,String dateStr,String nodeMatchStr,int colIndex) {
		try{
			
			XSSFSheet sheet = wworkbook.getSheet("6500 Parameters");
			if (sheet==null) {
				sheet = wworkbook.createSheet("6500 Parameters");
			}
	        ...
	        cell = row.createCell(colIndex+0);
	        cell.setCellValue("Node Name");;
	      
	        row.createCell(colIndex+1).setCellValue("Node IP");
	        row.createCell(colIndex+2).setCellValue("Description");
	        row.createCell(colIndex+3).setCellValue("Port");
	        row.createCell(colIndex+4).setCellValue("Parameter");
	        row.createCell(colIndex+5).setCellValue("Value");
	    	...
	    	connection = dbObj.getDBConnection();
			String query = "select node_name,node_ip,port_id,parameter_name,parameter_value from optical_reports.amazon_6500_perf_current where collection_date='"+dateStr+"' and node_name like '%"+nodeMatchStr+"%' order by node_name,port_id,parameter_name";
			//			logger.info(query);
			...
			while (result.next()) {
				...
				String description = nodeMatchStr + nodeName.substring(9, 14);

				if (nodeName.endsWith("S1")) {
					description += "X Path";
				} else if (nodeName.endsWith("S2")) {
					description += "Y Path";
				}

				if (portId.endsWith("6")) {

					if (parameterName.equals("GAIN")) {
						description += " Client Gain";
					}

					if (parameterName.equals("OPOUT-OTS")) {
						description += " Line Tx";
					}

					if (parameterName.equals("OPIN-OTS")) {
						description += " Client Rx";
					}

				}

				if (portId.endsWith("8")) {
				...
				if (parameterName.equals("ORL-OTS")) {
					description += " ORL";
				}
				
				if (parameterName.equals("SPAN LOSS")) {
	            ...
	            cell = row.createCell(colIndex+4);
	            cell.setCellValue(parameterName);
	            
	            cell = row.createCell(colIndex+5);
	            cell.setCellValue(parameterValue);
	            rowcount++;
			}
			
	        for (int i = colIndex+0; i < colIndex+6; i++) {
	            sheet.autoSizeColumn(i);
	        }
	...
	public void create6500PerformanceSpanLoss(XSSFWorkbook wworkbook,String dateStr) {
		create6500PerformanceSpanLoss(wworkbook,dateStr,"BOM53",0);
		create6500PerformanceSpanLoss(wworkbook,dateStr,"BOM59",9);
	}
	
	public void create6500PerformanceSpanLoss(XSSFWorkbook wworkbook,String dateStr,String nodeMatchStr,int colIndex) {
		try{
			
			XSSFSheet sheet = wworkbook.getSheet("6500 Span Loss");
			if (sheet==null) {
				sheet = wworkbook.createSheet("6500 Span Loss");
			}
	        ...
	        row.createCell(colIndex+3).setCellValue("Port");
	        row.createCell(colIndex+4).setCellValue("Parameter");
	        row.createCell(colIndex+5).setCellValue("Design Value");
	        row.createCell(colIndex+6).setCellValue("Value");
			...
			DbConnection  dbObj = new DbConnection();
	    	connection = dbObj.getDBConnection();
			String query = "select node_name,node_ip,port_id,parameter_name,parameter_value from optical_reports.amazon_6500_perf_current where collection_date='"+dateStr+"' and node_name like '%"+nodeMatchStr+"%' and parameter_name='SPAN LOSS' order by node_name,port_id,parameter_name";
			//			logger.info(query);
			logger.info("query is " + query);
			...
			while (result.next()) {
				...
				if (description.endsWith("PP4-X Path") ) {
					designValue=22.28;
				} else if (description.endsWith("PP4-Y Path")) {
					designValue=22.13;
				} else if (description.endsWith("PP5-X Path")) {
					designValue=22.16;
				} else if (description.endsWith("PP5-Y Path")) {
					designValue=22.17;
				}
	            ...
	            cell = row.createCell(colIndex+5);
	            cell.setCellValue(designValue);
	            
	            cell = row.createCell(colIndex+6);
	            cell.setCellValue(parameterValue);
	            rowcount++;
			}
			
	        for (int i = colIndex+0; i < colIndex+7; i++) {
	            sheet.autoSizeColumn(i);
	        }
	...
	public void create6500pm(XSSFWorkbook wworkbook,String dateStr) {
		
		XSSFSheet sheet = wworkbook.createSheet("6500 PM-BOM53-BOM59-PP4");
	
		int rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[0],0,0);
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[1],rowcount+2,0);
		
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[2],0,9);
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[3],rowcount+2,9);
		
		sheet = wworkbook.createSheet("6500 PM-BOM53-BOM59-PP5");
		
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[4],0,0);
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[5],rowcount+2,0);
		
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[6],0,9);
		rowcount = addRowsFor6500pm(sheet,dateStr,ipList6500[7],rowcount+2,9);
		
	}
	
	public int addRowsFor6500pm(XSSFSheet sheet,String dateStr,String ip,int rowcount,int cellIndex) {
		try{
			...
			DbConnection  dbObj = new DbConnection();
	    	connection = dbObj.getDBConnection();
			String query = "select node_name,node_ip,port_id,parameter_name,parameter_value,location,direction from optical_reports.amazon_6500_pm where collection_date='"+dateStr+"' and node_ip='"+ip+"' order by node_name,port_id,parameter_name";
			//			logger.info(query);
			logger.info("query is " + query);
			...
			while (result.next()) {
				 ...
				 nodeName = result.getString("node_name");
				 nodeIp = result.getString("node_ip");
				 portId = result.getString("port_id");
				 location = result.getString("location");
				 direction = result.getString("direction");
				 parameterName = result.getString("parameter_name");
				 parameterValue  = result.getString("parameter_value");
	...
	public void createWsPerformanceCurrent(XSSFWorkbook wworkbook,String dateStr,String sheetname,String wsIps,int colIndex) {
		try{
			XSSFSheet sheet = wworkbook.getSheet(sheetname);
			
			if (sheet == null) {
				sheet = wworkbook.createSheet(sheetname);
			}
	        ...
	        row.createCell(colIndex+0).setCellValue("Node Name");;
	        row.createCell(colIndex+1).setCellValue("Node IP");
	        row.createCell(colIndex+2).setCellValue("Port");
	        row.createCell(colIndex+3).setCellValue("Parameter");
	        row.createCell(colIndex+4).setCellValue("Value");
	        rowcount++;
			...
			DbConnection  dbObj = new DbConnection();
	    	connection = dbObj.getDBConnection();
			String query = "select node_name,node_ip,port_id,parameter_name,parameter_value from optical_reports.amazon_ws_perf_current where collection_date='"+dateStr+"' and node_ip in ("+wsIps+") order by node_ip,port_id";
			//			logger.info(query);
			logger.info("query is " + query);
...
//	            if (rowcount+Integer.parseInt(portNumber[1]) == 10) rowcount=11;
	...
	public void createWsPerformanceHistory(XSSFWorkbook wworkbook,String dateStr,String sheetname,String wsIp1, int colIndex) {
		XSSFSheet sheet = wworkbook.getSheet(sheetname);
		if (sheet == null) {
			sheet = wworkbook.createSheet(sheetname);
		}
		...
		int nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-1","optical-power",sheet,0,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-1","modem-performance",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-1.1","odu-performance",sheet,nextRowIndex,colIndex);
		
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-2","optical-power",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-2","modem-performance",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"1-2.1","odu-performance",sheet,nextRowIndex,colIndex);
	
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-1","optical-power",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-1","modem-performance",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-1.1","odu-performance",sheet,nextRowIndex,colIndex);
		
	
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-2","optical-power",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-2","modem-performance",sheet,nextRowIndex,colIndex);
		nextRowIndex = addRowsForWsPerformanceHistory(wworkbook,dateStr,wsIp1,"2-2.1","odu-performance",sheet,nextRowIndex,colIndex);

		} catch(Exception e) {
			logger.info("Exception occurred while adding WsPerformanceHistory",e);
		}
...
*/
	...
	public int addRowsForWsPerformanceHistory(XSSFWorkbook wworkbook,String dateStr,String wsIp1,String portName, String profileType,XSSFSheet sheet,int rowIndex, int cellIndex) {
		...
		try{
			...
			String query = "SELECT node_name, node_ip, configured_mode, interface_name, interface_type, profile_type, collection_start_time, "
					...
					+ "lane_number, bin_name, parameter_name, parameter_value, parameter_idf, collection_date, updated_time FROM optical_reports.amazon_ws_perf_history where collection_date='"+dateStr+"' and node_ip ='"+wsIp1+"' and interface_name ='"+portName+"' and profile_type ='"+profileType+"'  order by bin_name";
			//			logger.info(query);
			logger.info("query is " + query);
			...
			String recordBinName ="";

			while (result.next()) {
				
				recordBinName = result.getString("bin_name");
	...
	public void createWsCurrentAlarms(XSSFWorkbook wworkbook,String dateStr) {
		try{
			...
			String query = "SELECT node_name, node_ip, alarm_instance_id, acknowledged, intermittent, alarm_table_id, severity, local_date_time, "
					+ "instance_name, description, site_identifier, group_identifier, member_identifier, collection_date, updated_time FROM optical_reports.amazon_ws_alarms_current where collection_date='"+dateStr+"' order by node_ip";
			//			logger.info(query);
			logger.info("query is " + query);
...
//	            if (rowcount+Integer.parseInt(portNumber[1]) == 10) rowcount=11;
	...
	public void create6500CurrentAlarms(XSSFWorkbook wworkbook,String dateStr) {
		try{
			...
			String query = "SELECT node_name, node_ip, instance_aid, aid_type, severity, condition_type, service_affecting, alarm_date_time, location, direction, "
					...
					+ "FROM optical_reports.amazon_6500_alarms_current where collection_date='"+dateStr+"' order by node_ip";
...
//	            if (rowcount+Integer.parseInt(portNumber[1]) == 10) rowcount=11;
	...
	public void create6500HistoryAlarms(XSSFWorkbook wworkbook,String dateStr) {
		...
		try{
	        ...
	        for (int i=0;i < ipList6500.length;i++) {
	        	rowcount = addRows6500HistoryAlarms(wworkbook,sheet,startDateStr,endDateStr,ipList6500[i],rowcount+i*5);
	        }
		//addRows6500HistoryAlarms(wworkbook,sheet,startDateStr,endDateStr,ipList6500[1],rowcount+5);
		} catch(Exception e) {
			e.printStackTrace();
		}
		...
		if (response != null) {
			...
			if (jsonObj != null && jsonObj.getString("Result").equals("Success")) {
				...
				for (int i = 0; i < alarmsList.length(); i++) {
					...
					//logger.info(alarmObj.getString("atagSeq") + alarmObj.getString("conditionType")
						//	+ alarmObj.getString("severity"));
					nodeName = nodeNameMap.get(ip);
					nodeIp = alarmObj.getString("deviceip");
					severity = alarmObj.getString("severity");
					impact = alarmObj.getString("serviceAffected");
					component = alarmObj.getString("aid");
					raiseTime = alarmObj.getString("alarmDate");
					clearTime = alarmObj.optString("clearedTime");
					description = alarmObj.getString("conditionType");
					acknowledged = "";
					annotation = "";
					additionalText = alarmObj.getString("conditionDescription");
					;
					alarmType = alarmObj.getString("subType");
					probableCause = "";
					equipmentType = "";
					clearedBy = "";
					clfi = "";
	...
	public void createOPSStatus(XSSFWorkbook wworkbook,String dateStr) {
		try{
			...
			DbConnection  dbObj = new DbConnection();
	    	connection = dbObj.getDBConnection();
			String query = "SELECT node_name, node_ip, facility_id, scheme, memeber_type, remote_standard, facility_state, switch, reason, collection_date, updated_time FROM optical_reports.amazon_6500_wps_status where collection_date='"+dateStr+"' order by node_ip";
			//			logger.info(query);
			logger.info("query is " + query);
	...
	public void loadNodeNameMap() {
		
		try {
		
		DbConnection  dbObj = new DbConnection();
		//Connection connection = dbObj.getDBConnection();
    	Connection connection = dbObj.getDBConnectionMCPE();
		String query = "select ip,hostname,type from waveserver_devices";
		//			logger.info(query);
		logger.info("query is " + query);
		...
		while (result.next()) {
			...
			String nodeName = result.getString("hostname");
			String nodeIp = result.getString("ip");
			String nodeType = result.getString("type");
			
			nodeNameMap.put(nodeIp, nodeName);
			
			if (nodeType.equals("6500F")) {
			ignorePortParameters.add(nodeName+":AMP-1-4-6:OPIN-OTS");
			ignorePortParameters.add(nodeName+":AMP-1-4-8:OPOUT-OTS");	
			ignorePortParameters.add(nodeName+":AMP-1-6-6:OPIN-OTS");
			ignorePortParameters.add(nodeName+":AMP-1-6-8:OPOUT-OTS");
			
			
			wpsPorts.add(nodeName+":OPTMON-1-6-13");
			wpsPorts.add(nodeName+":OPTMON-1-6-15");
		...
		logger.info("Node Names : "+nodeNameMap);
				
		logger.info("ignorePortParameters : "+ignorePortParameters);
		logger.info("wpsPorts : "+wpsPorts);
		
		dbObj.closeConnection(connection);
		
		} catch(Exception e) {
			logger.error("Unable to load node names from db",e);
		}
	...
	public void finishPolling() {
		...
		ServiceNowRestClient.postAmazonReportToServiceNow(dateStr);
		
		
		if (CienaProperties.get("ciena.amazonhealthreport.email.enabled").equals("true")) {
			sendReportEmail(dateStr);
		}
	...
	private void sendReportEmail(String dateStr) {
		...
		String fileName = "amazon_health_report_" + dateStrFileName + ".xlsx";
		
		String fileNameWithPath = filePath + File.separator + fileName;

		
		String emailContent = "Please find the HealthCheckupReport generated on "+dateStr;
				
				
		smsEmailClient.sendEmailWithAttachment(dateStr, emailContent, fileName, fileNameWithPath);
